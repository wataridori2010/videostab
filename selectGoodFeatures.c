...
static void _sortPointListBlock(int *pointlist,int npoints, int divn){
    
    
    int npoints_block=npoints/divn;
    
    for (int i=0;i<divn; i++){
#ifdef KLT_USE_QSORT
        qsort(pointlist+3*npoints_block*i, npoints_block, 3*sizeof(int), _comparePoints);
#else
        _quicksort(pointlist+3*npoints_block*i, npoints_block);
#endif
    }
    int *pointlist_new=(int*)calloc(3*npoints,sizeof(int));
    
    // pick elem 1 each from blocks
    for (int j=0;j<npoints_block; j++){
        for (int i=0;i<divn; i++){
            pointlist_new[3*(i+divn*j)+0]=pointlist[3*(npoints_block*i+j)+0];
            pointlist_new[3*(i+divn*j)+1]=pointlist[3*(npoints_block*i+j)+1];
            pointlist_new[3*(i+divn*j)+2]=pointlist[3*(npoints_block*i+j)+2];
        }
    }
    memcpy(pointlist, pointlist_new,npoints*3*sizeof(int));
    free(pointlist_new);
...
//  }
     ...
     of the two eigenvalues of the Z matrix */
  {
    ...
    // block
    int DIVNUM = 4;
    int block_y = (nrows - 2 * bordery) / DIVNUM;
    int block_x = (ncols - 2 * borderx) / DIVNUM;
      
    for (int y0 = bordery ; y0 < nrows - bordery ; y0 += block_y){
        for (int x0 = borderx ; x0 < ncols - borderx ; x0 += block_x)  {
            
            for (y = y0 ; y < y0 + block_y; y += tc->nSkippedPixels + 1){
                for (x = x0 ; x < x0 + block_x ; x += tc->nSkippedPixels + 1)  {
...
//      for (x = borderx ; x < ncols - borderx ; x += tc->nSkippedPixels + 1)  {
  ...
  /* Sort the features from all blocks */
  _sortPointListBlock(pointlist, npoints);
